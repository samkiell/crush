...
const CollapseSection = ({ title, icon: Icon, children, defaultOpen = false }) => {
    ...
    return (
        <div className="collapse collapse-arrow bg-base-100 border border-base-200 rounded-2xl shadow-sm mb-3 overflow-hidden">
            <input
                type="checkbox"
                checked={isOpen}
                ...
                className="peer"
            />
            <div className="collapse-title text-lg font-medium flex items-center gap-3 py-4 peer-checked:bg-base-200/50 transition-colors">
                <div className="p-2 bg-primary/10 rounded-lg text-primary">
                    <Icon className="w-5 h-5" />
                </div>
                {title}
            </div>
            <div className="collapse-content bg-base-100 peer-checked:pt-4 peer-checked:pb-4">
                <AnimatePresence>
                    {isOpen && (
                        <motion.div
